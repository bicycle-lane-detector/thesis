...
zunächst einen Überblick der in diesem Kapitel erarbeiteten Inhalte, um das Vorgehen, 
...
vorgenommen wird. Mit dem Wissen aus dem Grundlagenkapitel können die Architekturen für die zu vergleichenden Modelle entworfen 
und deren Hyperparameter festgelegt werden. Schließlich können die Trainings- und Pre-Trainingsmethodik und die daraus resultierenden 
...
\begin{itemize}
	...
	\item \textbf{Radstreifen:} Ein Radstreifen ist nicht eigenständig, sondern steht in Bezug zu einer Straße. 
	Koordinaten eines Radstreifens entsprechen der Mittellinie der dazugehörigen Straße. 
...
Zunächst werden die Wege gefunden, die im derzeitigen Bild $f$ liegen. Dazu werden die Koordinaten der 
...
Die Maske wird an eine Liste angehangen und das nächste Foto bearbeitet. Der Datensatz wird dann aus dem Tupel  
der Listen der Fotos und Masken gebildet. Bei dem dargestellten Algorithmus handelt es sich um eine 
...
Innerhalb der Anfrage lässt sich zwischen einer Vielzahl von Attributen unterscheiden.
...
Die ausgelesenen Daten können anschließend als das Dateiformat GeoJSON exportiert werden. \\
...
\lstinputlisting[
	...
	caption={Auschnitt aus der GeoJSON, die die Overpass API zurückliefert. Es ist ein Radweg abgebildet.},
	captionpos=b,               % Position, an der die Caption angezeigt wird t(op) oder b(ottom)
	% style=EigenerPythonStyle,   % Eigener Style der vor dem Dokument festgelegt wurde
	% firstline=1,                % Zeilennummer im Dokument welche als erste angezeigt wird
	% lastline=50                 % Letzte Zeile welche ins LaTeX Dokument übernommen wird
...
Es wird die durchschnittliche Breite einer Fahrbahn von $17.625px$ angenommen. 
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Procedure{korrigiereStreifen}{$w$} \Comment{$w.typ = Radstreifen \land w.seite = rechts$}
			\State{$neueStützpunkte: liste$}
			\For{$i < |w.stützpunkte|$}
				\State{$v_i, v_{i+1} \in w.stützpunkte$}
				\State{$\Delta v \gets v_{i+1} - v_i$}
				\State{$e \gets \frac{1}{\norm{ \Delta v}_2} \Delta v$}
				\State{$r \gets \begin{pmatrix} 
					0 & 1  \\ 
					-1 & 0  
					\end{pmatrix} e$} \Comment{Rotiere $e$ um 90° nach rechts}
				\State{$r_s \gets \frac{1}{2} \cdot w.spurenAnzahl \cdot SPURENBREITE \cdot r$}
				\State{$neueStützpunkte.anhängen(v_i + r_s)$}
				\State{$neueStützpunkte.anhängen(v_{i+1} + r_s)$}
			\EndFor
			\State{$w.stützpunkte \gets neueStützpunkte$}
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
sondern die Fahrrad\textit{streifen}, denn diese liegen durch die Schätzung ggf. daneben oder verursachen die beschriebenen Sprünge. \\
...
Außerdem ist es dadurch nicht möglich zu erkennen, ob bei einer Kreuzung ein separater Streifen für linksabbiegende 
...
vorkommen und die lange zusammenhängende Struktur, die zu einem Radweg gehört, schwieriger zu erlernen ist. 
...
Hierbei sind zwei Dinge anzumerken, die nicht aus der Tabelle hervorgehen: 
...
Jede der folgenden Augmentationen, wie beispielhaft zu sehen in \autoref{fig:color-aug} und \autoref{fig:augmentation}, 
...
Die \ac{BIoU} adressiert das Verschiebungsproblem der \ac{IoU} mit einem Puffer zur relaxierten Ermittlung der 
...
sehr verzerrt. Zusammenfassend ist die BIoU ein Maß, das geeigneter der menschlichen Intuition nahekommt.
...
um die Ergebnisse zu validieren und früh Fehler in den eigenen Entscheidungen und Implementationen zu entdecken.
...
\begin{itemize}
	...
	sondern eine mit lediglich einem Output-Neuron pro Pixel, wobei keine Aktivierung einen Hintergrundpixel 
	...
	numerisch zu große Aktivierungen werden vermieden. Hierzu wird \ac{ELU} verwendet.\footnote{Eine genauere Beschreibung liegt im Implementationskapitel vor.}
...
Dieser Abschnitt beschreibt, warum die Backbone-U-Nets, wie in \autoref{sec:pretrained-as-unet-backbone} beschrieben,
...
für den ImageNet-Datensatz - verbessern sollen und nachvollzogen werden soll, ob diese Anpassungen 
für den konkreten Anwendungsfall der Radwegerkennung dienlich sind. Zudem ist VGG16 sehr beliebt als vortrainiertes 
Netz, weswegen es viele Vergleichsmöglichkeiten gibt. 
Des Weiteren wird VGG16 als Backbone in Research zum Pre-Training mit U-Nets eingesetzt (s. \autoref{sec:transfer-learning}), 
...
im Gebiet der Straßenerkennung darstellen und viele der in diesem Kapitel vorgenommenen Entscheidungen 
...
darauf nicht zur Verfügung stehen. 

Der DeepGlobe-Datensatz kann ohne Weiteres übernommen und nach den Regeln in \autoref{sec:pre-processing} vorverarbeitet werden. \\ 
...
Flächen \textit{nicht} zu schwärzen. 
Ansonsten unterliegen die Bilder dem gleichen Pre-Processing wie die sonstigen Datensätze (vgl. \autoref{sec:pre-processing}). \\
Schließlich werden beide Datensätze gemischt und in einen Training-Validation-Test-Split unterteilt (s. \autoref{tab:combined-split}). 
...
\begin{figure}
	...
	zu denen kein Bild vorhanden ist.}
...
Die \autoref{acro:BUNet}-Netze werden dabei nach den Regeln aus \autoref{sec:training} von Grund auf neu trainiert, 
wobei für die Backbone-Netze folgende Form des Fine-Tunings angewandt wird (vgl. \autoref{sec:transfer-learning}): 
Der Encoder der Backbone-Netze ist auf ImageNet vortrainiert und für die ersten 12 Epochen des Trainings eingefroren, 
...
Generell ist es auch möglich, unüberwachtes Lernen auf die Straßendatensätze (bzw. irgendwelche 
...
Wege relevant sind, wie die Farbe, die Straßenmarkierungen, die Breite im Verhältnis zur Länge und so weiter. 
...
Verbessert sich der Validation-Verlust nach sieben Epochen nicht, wird der beste Stand, die Epoche mit dem geringsten 
...
Es wird mit maximal 100 Epochen trainiert.
...
Abzweigende Pfeile bedeuten, dass der Input dupliziert wird und beide Wege verfolgt werden. 
...
vortrainiert. Diese vortrainierten \acp{BUNet} werden jeweils fine-tuned, indem einmal die linke Hälfte des 
Netzes (der Encoder) und einmal die rechte Hälfte (der Decoder) eingefroren, also als nicht modifizierbar gesetzt, werden.
Das Training wird so 12 Epochen vollzogen, dann wird die Lernrate verkleinert um Faktor 10 und daraufhin weiter 
das \textit{ganze} Netz trainiert, bis es zum vorzeitigen Ende kommt oder die 100. Epoche erreicht ist. \\

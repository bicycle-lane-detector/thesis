...
Ein \ac{NN} besteht aus mehreren einzelnen Neuronen, die miteinander verbunden werden.
...
\end{align}
Liegt der Wert über dem Schwellwert der Aktivierungsfunktion, wird das nachfolgende Neuron aktiviert.
...
Das \ac{NN} wird mithilfe der Testdaten kontrolliert, die Validierungsdaten können für eine Hyperparameter-Anpassung verwendet werden.
...
Die Größe der Reduktion ist abhängig von der gewählten Kernelgröße, sowie dem Stride, der jeweiligen Verschiebung des Kernels.
\ac{CNN}s werden häufig bei Bildern eingesetzt, da eine räumliche Beziehung zwischen einzelnen Bildpunkten Berücksichtigung findet, räumlich zusammengehörige Regionen werden verbunden.
...
Über eine vollvermaschte Schicht nach der letzten Pooling-Schicht kann z.B. eine Klassfikation gelernt werden.
...
sich jeden weiteren Block mit zum Mittel-Block verdoppeln, der dann 1024 Filter
besitzt. Daraufhin halbieren sich die Filter mit jedem weiteren Decoder-Block
wieder.
Das U-Net hat ca. 24 Mio. Parameter. \\
Ein Problem, welches im ursprünglich U-Net-Paper erwähnt wird, ist dass unter \ac{ReLU} ganze Netzteile dauerhaft inaktiv 
werden, aufgrund des Dying-Neuron-Problem (vgl. \autoref{sec:activation:relu}) \cite{Ronneberger.18052015}.
...
\begin{enumerate}
	...
	auf Satellitenbildern gut zu erkennen sind, deren unterschiedliche, sehr heterogenen Bestandteile jedoch weniger, 
	...
	((semi-)automatisch) zu erstellen auch für Radwege angewandt werden kann und wie die Güte eines solchen Datensatzes bewertet werden kann. 
	...
	zu ermitteln, dass einer existiert und wo dieser grob liegt (auf welcher Straßenseite). Für diesen Verwendugnszweck gibt 
	...
	keine Implementation auf Rastern, wie einer semantischen Segmentierung, sondern nur auf daraus extrahierten Graphen. 
